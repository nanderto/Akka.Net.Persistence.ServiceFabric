...
public static class AsyncEnumerableEx
{
    ...
    private static Fabric.IAsyncEnumerable<T> Create<T>(Func<Fabric.IAsyncEnumerator<T>> getEnumerator)
    {
        return new AnonymousAsyncEnumerable<T>(getEnumerator);
    }

    private class AnonymousAsyncEnumerable<T> : Fabric.IAsyncEnumerable<T>
    {
        private readonly Func<Fabric.IAsyncEnumerator<T>> _getEnumerator;

        public AnonymousAsyncEnumerable(Func<Fabric.IAsyncEnumerator<T>> getEnumerator)
        {
            _getEnumerator = getEnumerator;
        }

        public Fabric.IAsyncEnumerator<T> GetEnumerator()
        {
            return _getEnumerator();
        }
    }

    private static Fabric.IAsyncEnumerator<T> Create<T>(Func<CancellationToken, Task<bool>> moveNext, Func<T> current,
    ...
    private static Fabric.IAsyncEnumerator<T> Create<T>(Func<CancellationToken, Task<bool>> moveNext, Func<T> current, Action dispose)
    {
        return new AnonymousAsyncEnumerator<T>(moveNext, current, dispose);
    }

    private class AnonymousAsyncEnumerator<T> : Fabric.IAsyncEnumerator<T>
    {
    ...
    public static Fabric.IAsyncEnumerable<TResult> SelectAsync<TSource, TResult>(this IAsyncEnumerable<TSource> source, Func<TSource, int, Task<TResult>> selector)
    {
        if (source == null)
            throw new ArgumentNullException(nameof(source));
        if (selector == null)
    ...
    public static Fabric.IAsyncEnumerable<TSource> WhereAsync<TSource>(this IAsyncEnumerable<TSource> source, Func<TSource, Task<bool>> predicate)
    {
        if (source == null)
            throw new ArgumentNullException(nameof(source));
        if (predicate == null)
    ...
    public static Fabric.IAsyncEnumerable<TResult> SelectManyAsync<TSource, TResult>(this Fabric.IAsyncEnumerable<TSource> source, Func<TSource, Task<IAsyncEnumerable<TResult>>> selector)
    {
        if (source == null)
            throw new ArgumentNullException(nameof(source));
        if (selector == null)
    ...
    public static async Task ForEachAsyncAsync<TSource>(this Fabric.IAsyncEnumerable<TSource> source, Func<TSource, int, Task> action, CancellationToken cancellationToken)
    {
        if (source == null)
            throw new ArgumentNullException(nameof(source));
        if (action == null)
    ...
    public static Fabric.IAsyncEnumerable<T> AsAsyncEnumerable<T>(this Fabric.IAsyncEnumerable<T> enumerable)
    {
        return new AsyncEnumerableFabricWrapper<T>(enumerable);
    }

    private sealed class AsyncEnumerableFabricWrapper<T> : Fabric.IAsyncEnumerable<T>
    {
        ...
        private class AsyncEnumeratorFabricWrapper : Fabric.IAsyncEnumerator<T>
        {
...
{
	public static async Task<Fabric.IAsyncEnumerable<KeyValuePair<TKey, TValue>>> CreateLinqAsyncEnumerable<TKey, TValue>(
	...
	public static async Task<Fabric.IAsyncEnumerable<T>> CreateLinqAsyncEnumerable<T>(
