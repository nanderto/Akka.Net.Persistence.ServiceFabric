    ...
    public class DeliveryActor : UntypedActor
    {
        ...
        protected override void OnReceive(object message)
        {
            ...
            if (message is Confirmable)
            {
                ...
                if (Confirming)
                {
                    ServiceEventSource.Current.Message("Confirming delivery of message id: {0} and data: {1}", msg.DeliveryId, msg.Data);
                    Context.Sender.Tell(new Confirmation(msg.DeliveryId));
                }
                else
                {
                    ServiceEventSource.Current.Message("Ignoring message id: {0} and data: {1}", msg.DeliveryId, msg.Data);
                }
            }
        }
    }
    ...
    public class AtLeastOnceDeliveryExampleActor : AtLeastOnceDeliveryActor
    {
        ...
        protected override bool ReceiveRecover(object message)
        {
            if (message is Message)
            {
                ...
                ServiceEventSource.Current.Message("recovered {0}",messageData);
                Deliver(DeliveryPath,
                        id =>
                        {
                            ServiceEventSource.Current.Message("recovered delivery task: {0}, with deliveryId: {1}", messageData, id);
            ...
            else if (message is Confirmation)
            {
                ...
                ServiceEventSource.Current.Message("recovered confirmation of {0}", deliveryId);
            ...
            else if (message is Message)
            {
                Persist(message as Message, m =>
                {
                    Deliver(DeliveryPath,
                        id =>
                        {
                            ServiceEventSource.Current.Message("sending: {0}, with deliveryId: {1}", m.Data, id);
